...
public class CaptureHelpers {
    ...
    public static void captureScreenshot(WebDriver driver, String screenName) {
        ...
        try {
            ...
            File theDir = new File(PropertiesFile.getPropValue("exportCapturePath"));
            if (!theDir.exists()){
                theDir.mkdirs();
            }
            // Chổ này đặt tên thì truyền biến "screenName" gán cho tên File chụp màn hình
            FileHandler.copy(source, new File( PropertiesFile.getPropValue("exportCapturePath") + "/" + screenName + "_" + dateFormat.format(new Date()) + ".png"));
